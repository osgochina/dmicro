...
func (that *serverHandler) handler(conn *websocket.Conn) {
	...
	if err != nil {
		logger.Errorf("serverHandler: %v", err)
		return
	}
	// 执行钩子
	if stat := that.afterAccept(sess, conn); !stat.OK() {
		if err := sess.Close(); err != nil {
			logger.Errorf("sess.Close(): %v", err)
		}
		return
	}
	// 等待会话关闭，结束该协程
...
func (that *serverHandler) beforeHandshake(r *http.Request) (stat *drpc.Status) {
	...
	defer func() {
		if p := recover(); p != nil {
			logger.Errorf("[BeforeWebsocketHandshakePlugin:%s] addr:%s, panic:%v", pluginName, r.RemoteAddr, p)
	...
	for _, plugin := range p.GetAll() {
		if _plugin, ok := plugin.(BeforeWebsocketHandshakePlugin); ok {
			...
			if stat = _plugin.BeforeHandshake(r); !stat.OK() {
				logger.Debugf("[BeforeWebsocketHandshakePlugin:%s] addr:%s, error:%s", pluginName, r.RemoteAddr, stat.String())
				return stat
			}
		}
	}
...
func (that *serverHandler) afterAccept(sess drpc.Session, conn *websocket.Conn) (stat *drpc.Status) {
	...
	defer func() {
		if p := recover(); p != nil {
			logger.Errorf("[AfterWebsocketAcceptPlugin:%s] network:%s, addr:%s, panic:%v", pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), p)
			stat = statInternalServerError.Copy(p)
		}
	}()
	// 查找是否有定义的钩子，有的话执行该钩子
	for _, plugin := range p.GetAll() {
		if _plugin, ok := plugin.(AfterWebsocketAcceptPlugin); ok {
			...
			if stat = _plugin.AfterAccept(sess, conn); !stat.OK() {
				logger.Debugf("[AfterWebsocketAcceptPlugin:%s] network:%s, addr:%s, error:%s", pluginName, sess.RemoteAddr().Network(), sess.RemoteAddr().String(), stat.String())
				return stat
			}
		}
	}
