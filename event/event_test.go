...
func TestEvent(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		e1 := event.NewEvent("test1", nil)
		e1.SetName("test1")
		e1.SetData(map[interface{}]interface{}{
			"arg0": "val0",
		})
		e1.SetTarget("test_target")
		...
		t.Assert(e1.Target(), "test_target")
...
func TestAddEvent(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err := manager.AddEvent(&event.Event{})
		t.AssertNE(err, nil)

		_, ok := manager.GetEvent("evt1")
		t.Assert(ok, false)

		e := event.NewEvent("evt1", map[interface{}]interface{}{"k1": "val1"})
